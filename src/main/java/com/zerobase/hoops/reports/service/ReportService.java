...
public class ReportService {
  ...
  public String reportContents(String reportId) {
    ...
    ReportDocument reportDocument = reportRepository.findById(reportId)
        .orElseThrow(() -> new CustomException(ErrorCode.NOT_EXIST_REPORTED)
    );
  ...
  public Page<ReportListResponseDto> reportList(int page, int size) {
    ...
    Page<ReportDocument> reportPage =
        reportRepository.findByBlackListStartDateTimeIsNull(
  ...
  public void reportUser(ReportDto request) {
    ...
    UserDocument user = userRepository.findById(
            jwtTokenExtract.currentUser().getId())
        .orElseThrow(() -> new CustomException(
            ErrorCode.USER_NOT_FOUND));

    UserDocument reportedUser = userRepository.findById(
    ...
    long reportId = reportRepository.count() + 1;

    reportRepository.save(request.toEntity(user, reportedUser, reportId));
